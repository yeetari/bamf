...
bool run(Function *function, Graph<BasicBlock> *cfg, int *no_preds_pruned_count) {
    bool changed = false;
    for (auto &block : *function) {
        ...
        if (cfg->preds_of(block.get()).empty()) {
...
void CfgSimplifier::run_on(Function *function) {
    ...
    bool changed = false;
    do {
        changed = run(function, &cfg, &no_preds_pruned_count);
    } while (changed);
